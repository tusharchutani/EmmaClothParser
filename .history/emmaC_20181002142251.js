...
function getProductArray(productInfo){
	...
	var firstVariantSKU;
	const {productHandel, productTitle, variantPrice, body, productType, option1Name, option1Values, imgAlt, migratedFrom,imgSrc} = productInfo;
	var firstImage = imgSrc[0] ? imgSrc[0] : null;
	...
	if(productInfo.imgSrc.length == productInfo.option1Values.length){
		for(var i = 1; i < productInfo.imgSrc.length; i++){
			var variantSKU = productHandel+"-"+option1Values[i];
			var tempProduct = new Product({
				productHandel, 
				imgSrc:imgSrc[i],
				option1Name,
				variantSKU,
				option1Value:option1Values[i]
			});
			products.push(tempProduct);
		}

	}else if(productInfo.imgSrc.length > productInfo.option1Values.length){

	}else{ //productInfo.imgSrc.length > productInfo.option1Values.length
