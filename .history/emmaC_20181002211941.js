...
async function getAllHeadingLinks(){
	var links = [];
	try{
		var html = await rp({uri: 'https://www.emmacloth.com', headers: {Connection: 'keep-alive'}});
...
async function getAllProductNameAndUrl(url){
	...
	try{
	var html = await rp({uri: url, headers: {Connection: 'keep-alive'}});
	var numOfItems = 0;
...
async function getProductInfo(productUrl, otherOptionsVisisted=false){
	...
	var html = await rp({uri: productUrl, headers: {Connection: 'keep-alive'}});
	...
	for(var i = 0; i < ProductsArr.length; i++){
		...
		log.info("Adding product to DB"+ProductsArr[i].variantSKU + " link:"+productUrl);
	...
	//insert into csv
	await csvWrite.writeRecords(ProductsArr);
...
async function getAllProductsFromHeaderLink(headerLink){
	...
	var nextLink;
	try{
	var html = await rp({uri: headerLink, headers: {Connection: 'keep-alive'}});
	var numOfItems = 0;
